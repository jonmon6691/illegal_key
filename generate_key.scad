...
//
// Modified by jonmon6691 to accept arbitrary length bittings
//
...
// Run generate_bitting.py and paste the result here
bitting = [];

assert(len(bitting) > 0, "Key bitting is missing! You need to run 'generate_bitting.py' and paste the result into this file.");
...
module sc1(bits) {
    ...
    shoulder = mm(.231);
    pin_spacing = mm(.15625); // According to the dalco reference this should be 0.156, but I'm adding precision to bring it to exactly 2.5 sixteenths
    depth_inc = mm(.015);
    
    //length = mm(17/16); // "Correct" length
    length = mm((2.5*len(bits)/16)) + shoulder + mm(.15);
